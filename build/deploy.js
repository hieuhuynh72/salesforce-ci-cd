...
var logger = (function (fs) {
  ...
  function flush() {
    var logFile = path.resolve(
      (process.env.SALESFORCE_ARTIFACTS || ".") + "/DeployStatistics.log"
    );
...
fs.stat("./src/package.xml", function (err, stat) {
  if (err === null) {
    ...
    var artifacts = path.resolve(
      (process.env.SALESFORCE_ARTIFACTS || ".") + "/package.xml"
    );
...
if (isProduction) {
  ...
  console.info(
    `On '${branch}' branch represents production. Leaving loginUrl as-is.`
  );
} else {
...
options.username = "cxbe01@skedulo.com.dev";
options.password = "JA8HL9QjHdkGE4QoduE9";
